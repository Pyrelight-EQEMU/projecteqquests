sub EVENT_SAY {  
  ...
  my $cl_count = keys %{ { plugin::GetUnlockedClasses($client) } } || 1;
  ...
  if ($text=~/hail/i) {     
      if ($progress <= 0) {
        ...
        plugin::NPCTell("Hail, $charname. I sense the [$adventurers_soul] growing within you. Welcome to the Nexus, the best place in all the realms to start
                        your journey to [nurture] it.");
      } elsif ($client->GetLevel() >= 20 and $cl_count < 2) {
        if ($unlocksAvailable < 1) {
          plugin::NPCTell("I'm happy that you've returned, $charname. I sense that you've gained quite a bit of experience, enough for me to trust you with 
                          [additional power] and responsibility. Are you ready for your soul to properly grow?");
        } else {
          plugin::NPCTell("I've already unlocked your potential, $charname. You only need to seize it and [choose your class].");
        }
      } elsif ($client->GetLevel() > 45 && plugin::GetTotalLevels($client) >= 60 && $progress < 4) {
        plugin::NPCTell("Ah, $charname, just who I was about to look for. You've grown substantially in power, and it is time for me to call in my marker. My apprentice,
                        Theralon, has been asking for assistants recently. Go see him in The Bazaar, and learn how you might help him.");
        $client->SetBucket("MAO-Progress", 3);
      } else {
        plugin::NPCTell("It is good to see you again, $charname. I don't have anything for you right now, but return to me when you've gained 
    ...
    if ($cl_count < 2) {
      plugin::NPCTell("I think that you have the potential to go far, $charname, perhaps far enough to help me accomplish some goals that I have not yet succeeded at alone.
      ...
      plugin::NPCTell("I've already assisted you with your first soul expansion, $charname.");
    }
  }

  elsif ($text=~/additional Class/i and $cl_count < 2 and $unlocksAvailable < 1) {    
    ...
    if (plugin::UnlockClass($client, $class{$text})) {
      ...
      $client->SetBucket("MAO-Progress", 2);
    }
  }
}