my $expedition_name = "Feat of Strength: Nagafen's Lair";
my $dz_zone         = "soldungb";
my $reward          = 1;
my $dz = undef;
my $min_players     = 1;
...
my $explain_message = "Inside is big-big dragon, Lord Nagafen. KILL! Master also wants the soul-energy of the giants Rokyl, Tranix, and Skarlon!";
my @target_list     = (32022, 32041, 32020, 32040); # Array of npc_type IDs that we need to kill. Add multiple times for quantity.
my $target_level    = 52;
    ...
    if ($text =~ /hail/i ) {
        ...
        if (!(keys %{$client->GetExpeditionLockouts($expedition_name)})) {
            ...
            if (my $bucket = $client->GetBucket("FoS-$dz_zone")) {
                plugin::YellowText("You have previously completed this challenge. You may choose an escalation tier below, then respond to the goblin. Higher escalation tier will increase the challenge of the creatures in the instance, but also increase the potential rewards.");
